...
public interface EncounterNoteRepository extends  JpaRepository<EncounterNote, String>{
    ...
    @Query(value = "select * from encounternote where practitionerid is not null and visitid is not null and patientid  is not null order by id OFFSET ?1 LIMIT 100",nativeQuery = true)
