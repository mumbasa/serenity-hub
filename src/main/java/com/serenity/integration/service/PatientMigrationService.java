...
public void sumitTasker(int size){
    ...
    List<PatientData> patientData = patientRepository.findTop5();//stream().filter(e -> !ids.contains(e.getExternalId())).collect(Collectors.toList());
...
public int task(List<PatientData> data,List<String> ids){
 List<PatientData>  datas= data.stream().filter(e-> !ids.contains(e.getExternalId())).collect(Collectors.toList());
